...
def get_path(directname, filename_format):
    ...
    for item in file_list:   # for every filename in curr. dir.
        # obtain match object
        cur_filename = re.search(filename_format, item)
...
# after sorting electrodes using sort_elect() defined above, this function
# converts coordinates of every electrode contact from mni to image space
# GET_ELECT_CONTACT_IJK(SORTED_ELECT_ARRAY, IMG_HEADER):
# inputs:
# SORTED_ELECT_ARRAY is nested list of sorted electrodes from sort_elect()
# IMG_HEADER is the Nifti image header from which info of transformation
# matrix is obtained to convert coordinates from mni to ijk space
# outputs:
# ELE_SORTED_IJK is a nested list with name, coordinates in mni and image
# space available
# ELE_IJK_ALL is a single layer list with same info embedded as ELE_SORTED_IJK

def get_elect_contact_ijk(sorted_elect_array, img_header):

    # initialize list for storing coordinates of electrode contacts
    # in this nested list, every entry contains a list with format given below:
    # 1st entry = name tag of electrode
    # 2nd entry = list of electrode coordinates in mni space
    # 3rd entry = list of electrode coordinates in image (ijk) space
    # e.g. to obtain coordinates of contact of one type of electrode
    # ele_sorted_ijk[elect_type][i^th electrode contact]
    ele_sorted_ijk = []

    for i in range(len(sorted_elect_array)):  # for every type of electrode
        curr_list = []  # initialize list for storing info. of curr. ele. type
        for j in range(len(sorted_elect_array[i])):  # for every contact of curr. electrode type
            ele_name = sorted_elect_array[i][j][0]  # obtain electrode name
            ele_mni = sorted_elect_array[i][j][1:]  # obtain mni coordinates of curr. electrode
            # convert from mni to image space using image header, then round
            # number to nearest integer
            ijk = np.rint(mni2ijk(ele_mni, img_header))

            # append info. of current elect. contact to curr. list (curr. ele. type),
            # here ijk is a numpy array, convert it to list type
            curr_list.append([ele_name, ele_mni, ijk.tolist()])

        # after looping through all contacts in current electrode type,
        # append curr. list to nested list, ele_sorted_ijk)
        ele_sorted_ijk.append(curr_list)

    # nested list ele_sorted_ijk groups electrodes of same type into one element,
    # convert ele_sorted_ijk to single layer list, with same column spec.
    ele_ijk_all = []  # initialize list
    for i in range(len(ele_sorted_ijk)):  # for every type of electrode
        for j in range(len(ele_sorted_ijk[i])):  # for every contact of curr. elect. type
            ele_ijk_all.append(ele_sorted_ijk[i][j])  # append info. to list

    # assemble return statement: ele_sorted_ijk, ele_ijk_all
    return ele_sorted_ijk, ele_ijk_all
